...
async function slugsFrom(file) {
  ...
  return [...source.matchAll(/^\s*\['([^']+-2026-09-08)'/gm)].map((match) => match[1]);
}
const groups = [
  {family:'blog', expected:12, slugs:await slugsFrom('app/sep08-blog-batch.ts')},
  {family:'research', expected:5, slugs:await slugsFrom('app/sep08-research-batch.ts')},
];
...
for (const group of groups) {
  ...
  for (const slug of group.slugs) {
    ...
    assert(title && !titles.has(title), `${path} unique visible title`); titles.add(title);
    assert(!bodies.has(html), `${path} unique rendered body`); bodies.add(html);
    assert(html.includes('September 8, 2026'), `${path} visible date`);
    ...
    assert(imagePaths.length && ogImage, `${path} rendered and OG images present`);
    for (const image of new Set([...imagePaths, ogImage])) {
      ...
      assert.equal((await fetch(imageUrl)).status, 200, `${path} image ${image}`);
    }
  }
}
