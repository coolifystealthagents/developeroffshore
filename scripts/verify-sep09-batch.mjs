...
async function slugsFrom(file) {
  ...
  return [...source.matchAll(/^\s*\['([^']+-2026-09-09)'/gm)].map((match) => match[1]);
}
const groups = [
  {family:'blog', expected:12, slugs:await slugsFrom('app/sep09-blog-batch.ts')},
  {family:'research', expected:5, slugs:await slugsFrom('app/sep09-research-batch.ts')},
];
...
for (const group of groups) {
  ...
  for (const slug of group.slugs) {
    ...
    assert(html.includes('September 9, 2026'), `${path} visible date`);
